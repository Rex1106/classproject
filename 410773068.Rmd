...
```{r ans133}
sum(str_detect(subsample$課外活動,"團隊活動"))/43
```
...
```{r ans14}
3-is.na(hwGrades[,2])-is.na(hwGrades[,3])-is.na(hwGrades[,4])->hwGrades$交作業次數
hwGrades
```
...
classSurvey$學號[isFemale] -> 女學號
hwGradeSubsample$女性 <- hwGradeSubsample$學號 %in% 女學號
hwGradeSubsample
```
...
```{r ans18}
hwGradeSubsample[hwGradeSubsample$女性==FALSE,]
sum(hwGradeSubsample[hwGradeSubsample$女性==FALSE,6])/17
```
...
```{r ans19}
classSurvey$學號[hasGroupActivity] -> 有團隊活動的學號
hwGradeSubsample$有團隊活動 <- hwGradeSubsample$學號 %in% 有團隊活動的學號
mean(hwGradeSubsample$平均成績[(hwGradeSubsample$女性==T & hwGradeSubsample$有團隊活動==T)])->meanGradeFemaleWithAct
...
with_tz(hwSubmit$Timestamp,tz="Asia/Taipei")->hwSubmit$Timestamp
...
ymd_hms("2018-11-06 00:00:00",tz="Asia/Taipei")->deadline
...
```{r ans23}
as.numeric(hwSubmit3$截止時差)->hwSubmit3$截止時差
sum(hwSubmit3$女性==TRUE)
sum(hwSubmit3[hwSubmit3$女性==TRUE,5])/34->x
```

```{r ans23-1}
sum(hwSubmit3$女性==FALSE)
sum(hwSubmit3[hwSubmit3$女性==FALSE,5])/15->y
```
